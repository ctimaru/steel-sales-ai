...
export default async function PriceHistoryPage({ params }: { params: Promise<{ productId: string }> }) {
  ...
  if (!payload.found || !payload.product) {
    return (
      <div className="space-y-4">
        <p className="text-sm text-slate-600">Nessuno storico prezzi trovato per questo prodotto nel tenant attivo.</p>
        <Link href={`/products/${productId}`} className="text-sm font-semibold text-indigo-600">← Torna a Product 360</Link>
      </div>
    );
  }
  ...
  return (
    <div className="mx-auto max-w-7xl space-y-7">
      <header>
        <Link href={`/products/${productId}`} className="text-xs font-semibold text-indigo-600">← Product 360</Link>
        <div className="mt-3 flex flex-col justify-between gap-4 lg:flex-row lg:items-end">
          <div>
            ...
            <h1 className="mt-3 text-3xl font-semibold tracking-tight text-slate-950">Price History · {productLabel(product)}</h1>
      ...
      <section className="grid gap-4 lg:grid-cols-2">
        <article className="rounded-3xl bg-slate-950 p-5 text-white shadow-sm sm:p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="rounded-full bg-white/10 px-3 py-1 text-xs font-semibold">Quote</span>
          </div>
        ...
        <article className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
          <div className="flex items-start justify-between gap-4">
            ...
            <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">Order</span>
          </div>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">Quote e order price sono separati; RFQ/consegne con prezzo restano riferimenti e non vengono mescolati nei trend.</p>
          </div>
          <p className="text-xs text-slate-400">{payload.history?.length ?? 0} osservazioni con prezzo</p>
        </div>
            ...
            <tbody className="divide-y divide-slate-100">
              {(payload.history ?? []).map((row) => (
                <tr key={row.observation_id}>
                  ...
                  <td className="text-slate-600">{row.company?.name ?? row.company?.company_name ?? "Non strutturato"}</td>
                  <td>
                    <p className="text-slate-700">{row.delivery_term ?? "Non strutturato"}</p>
                  ...
                  <td className="text-right">{row.thread_id ? <Link href={`/conversations/${row.thread_id}`} className="font-semibold text-indigo-600">Evidence →</Link> : null}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      <section className="rounded-3xl border border-slate-200 bg-white p-5 shadow-sm sm:p-6">
        <div className="flex flex-wrap items-end justify-between gap-3">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500">Solo stessa famiglia e stessa qualità. Lo score è deterministico: norma, geometria e spessore; nessuna classificazione AI opaca.</p>
          </div>
          <span className="rounded-full bg-indigo-50 px-3 py-1 text-xs font-semibold text-indigo-700">score 0–100</span>
        </div>
        <div className="mt-5 overflow-x-auto">
          <table className="w-full min-w-[980px] text-left text-sm">
            <thead className="border-b border-slate-200 text-xs uppercase tracking-wide text-slate-400">
              <tr><th className="py-2">Prodotto</th><th>Tipo prezzo</th><th>Prezzo norm.</th><th>Score</th><th>Perché comparabile</th><th>Vs target</th><th></th></tr>
            </thead>
            <tbody className="divide-y divide-slate-100">
