-- P0.2 — Promote worker jobs into the tenant's commercial dataset rather than
-- requiring a dataset owned by the individual uploader.
...
begin
  ...
  if job.organization_id is null then
    raise exception 'Worker job % has no organization_id', target_job_id;
  end if;
  ...
  if job.dataset_id is not null and job.thread_id is not null then
    return jsonb_build_object(
      ...
      'review_count', (
        ...
        where rq.thread_id = job.thread_id
          and rq.organization_id = job.organization_id
      )
    );
  end if;
  ...
  where d.organization_id = job.organization_id
    and d.status in ('active', 'ready')
  order by (d.status = 'active') desc, d.created_at desc
  limit 1;

  if target_dataset_id is null then
    raise exception 'No active/ready commercial dataset for organization %', job.organization_id;
  end if;
  ...
  where o.thread_id = target_thread_id
    and o.organization_id = job.organization_id
