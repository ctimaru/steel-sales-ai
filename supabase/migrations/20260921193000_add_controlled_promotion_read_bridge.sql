-- PA2.3 — controlled promotion read-model bridge.
-- Adds a current promotion read model and a backward-compatible Global Search bridge.
-- No bulk backfill and no destructive switch away from commercial_observations.
...
where p.status = 'applied'
  and p.promotion_action in ('created','linked','updated')
  and not exists (
    select 1
    from public.commercial_entity_promotions s
    where s.supersedes_promotion_id = p.id
  );

revoke all on public.current_commercial_entity_promotions from public, anon, authenticated;
grant select on public.current_commercial_entity_promotions to service_role;
...
comment on function public.p1_global_structured_search_bridge(uuid,text,text[],jsonb,integer) is
  'PA2.3 controlled read bridge: normalized promoted business events replace their observation event; all unpromoted data remains on observation fallback.';
