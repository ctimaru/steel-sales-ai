...
as $$
  with params as (
    select
      ...
      websearch_to_tsquery('pg_catalog.simple'::regconfig, p_query_text) as ts_query
  ),
  active_model as (
...
revoke all on function public.hybrid_search_knowledge(
  uuid, text, real[], integer, integer, jsonb, jsonb, integer
...
comment on function public.hybrid_search_knowledge(
  ...
  'M5.5 owner-scoped hybrid retrieval: active-model vector similarity + full-text RRF + canonical entity signals + structured commercial/entity filters with provenance.';
