-- P0.2 — Preserve the existing worker/RAG signature while changing private
-- knowledge visibility from a single owner to the owner's active organization.
...
as $$
  ...
  requested_filter_entities as (
    ...
    join public.knowledge_entities e
     ...
     and (
       ...
       or e.organization_id = (select organization_id from tenant)
     )
     and (
  ...
  visible as (
    ...
    join public.knowledge_sources s
     ...
     and (
       ...
       or s.organization_id = (select organization_id from tenant)
     )
  ...
  order by s.rrf_score desc,
           s.vector_similarity desc nulls last,
           s.lexical_score desc nulls last
  limit (select match_count from params);
$$;

-- Retrieval is an internal worker capability. Keep the security-definer surface
-- unavailable to browser roles.
revoke execute on function public.hybrid_search_knowledge(uuid, text, real[], integer, integer, jsonb, jsonb, integer)
  from public, anon, authenticated;
grant execute on function public.hybrid_search_knowledge(uuid, text, real[], integer, integer, jsonb, jsonb, integer)
  to service_role;
