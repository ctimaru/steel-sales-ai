...
select jsonb_build_object(
  ...
  'signals',coalesce((select jsonb_agg(value) from rows),'[]'::jsonb),
...
revoke all on function public.p2_reengagement_signals(uuid,integer,integer,integer,integer)
  from public,anon;
grant execute on function public.p2_reengagement_signals(uuid,integer,integer,integer,integer)
  to authenticated,service_role;

comment on function public.p2_reengagement_signals(uuid,integer,integer,integer,integer) is
  'P2.1 tenant-safe deterministic re-engagement signals for verified commercial companies. No predictive score; explicit watch/dormant thresholds and evidence.';
