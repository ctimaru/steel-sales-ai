...
It always creates the final job `P0 Required Gate / required` on pull requests to `main` and pushes to `main`.
...
- changes to the gate/deployment configuration → all suites;
...
`main` must reject direct pushes and require pull requests with the check:

`P0 Required Gate / required`

Recommended repository rule:

1. require a pull request before merging;
2. require the `P0 Required Gate / required` status check;
3. require branch to be up to date before merging;
4. block force pushes and branch deletion;
5. allow repository administrators to bypass only for emergency recovery.
...
- branch protection requires the stable check;
- Vercel skips unaffected backend/database-only changes;
